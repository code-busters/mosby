...
(function ($) {
    ...
    $(document).on('change', '.btn-file :file', function () {
        ...
        readURL(this, '#background-block', 'background-image');
    ...
    $(document).on('change', '#open-logo:file', function () {
        var input = $(this),
            numFiles = input.get(0).files ? input.get(0).files.length : 1,
            label = input.val().replace(/\\/g, '/').replace(/.*\//, '');
        readURL(this, '.logo', 'src');
        input.trigger('fileselect', [numFiles, label]);
    });
    $(document).ready(function () {
        $('#open-logo:file').on('fileselect', function (event, numFiles, label) {
    ...
    $(document).on('change', '#open-profile-img:file', function () {
        var input = $(this),
            numFiles = input.get(0).files ? input.get(0).files.length : 1,
            label = input.val().replace(/\\/g, '/').replace(/.*\//, '');
        readURL(this, '.user-profile-img', 'background-image');
        input.trigger('fileselect', [numFiles, label]);
    });
    $(document).ready(function () {
