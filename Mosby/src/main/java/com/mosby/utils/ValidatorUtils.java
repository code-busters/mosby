		...
		for (Field field : type.getDeclaredFields()) {
			...
			if (field.isAnnotationPresent(Size.class)) {
				...
				if (value < annotation.min()) {
					errors.add(field.getName() + " must be > " + annotation.min());
				} 
				if (value > annotation.max()){
					errors.add(field.getName() + " must be < " + annotation.max());
				}
			...
			if (field.isAnnotationPresent(Min.class)) {
				...
				if (value < annotation.value()) {
					errors.add(field.getName() + " must be > " + annotation.value());
				} 
			...
			if (field.isAnnotationPresent(Password.class)){
				Password annotation = (Password) field.getAnnotation(Password.class);
				field.setAccessible(true);
				java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(annotation.pattern());
				Matcher mathcer = pattern.matcher(field.get(object).toString());
				if(!mathcer.matches()){
