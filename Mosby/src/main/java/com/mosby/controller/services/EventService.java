...
public class EventService {
	...
	public int create(HttpServletRequest request, HttpServlet servlet)
		...
		EventCategory eventCategory = new ReadGenericObjectService<>(
				EventCategory.class).readById(Integer.parseInt(request
				.getParameter("event_category")));
		EventType eventType = new ReadGenericObjectService<>(EventType.class)
				.readById(Integer.parseInt(request.getParameter("event_type")));

		Date startDate = null, endDate = null, startTime = null, endTime = null;

		try {
			startDate = new SimpleDateFormat(DATE_FORMAT).parse(request
		...
		if (request.getParameter("privacy_event") == null
				|| request.getParameter("privacy_event").equals("0")) {
			privacy = false;
		...
		if (idTicketsArray != null && idTicketsArray.length != 0) {
			ReflectionDao<TicketInfo> ticketInfoDao = new ReflectionDao<>(
					TicketInfo.class);
			for (String currInt : idTicketsArray) {
				...
				int maxNumber = Integer.parseInt(request
						.getParameter("event_ticket_quantity_" + currentId));
				String stringPrice = request.getParameter("event_ticket_price_"
						+ currentId);
				...
				try {
					...
					startTime = new SimpleDateFormat(TIME_FORMAT).parse(request
							.getParameter("ticket_start_time_" + currentId));
					endTime = new SimpleDateFormat(TIME_FORMAT).parse(request
				...
				TicketInfo ticketInfo = new TicketInfo(ticketInfoName, event,
						type, ticketDescription, maxNumber, price, startDate,
		...
		if (idPromoCodesArray != null && idPromoCodesArray.length != 0) {
			ReflectionDao<PromoCode> promoCodeDao = new ReflectionDao<>(
					PromoCode.class);
			for (String currInt : idPromoCodesArray) {
				...
				PromoCode promoCode = new PromoCode(event, code, discount,
	...
	public void updateEvent(HttpServletRequest request, HttpServlet servlet)
			throws IllegalStateException, IOException, ServletException {
		...
		Event updatedEvent = new Event(eventId, organizer, name, description,
				eventCategory, eventType, startDate, startTime, endDate,
				endTime, location, eventLogo, eventBackground, privacy);
	...
	public void updateTicketsInfo(HttpServletRequest request) {
		...
		Event event = new ReadGenericObjectService<>(Event.class)
				.readById(eventId);
		List<TicketInfo> currentTicketsInfoList = new ReadGenericObjectService<>(
				TicketInfo.class).readListByField("event_ref=", eventId);
		...
		if (idTicketsArray != null && idTicketsArray.length != 0) {
			for (String newId : idTicketsArray) {
				...
				if (currentIdTicketsInfoList.contains(newId)) {
					ticketInfo = new TicketInfo(currentId, ticketInfoName,
							event, type, ticketDescription, maxNumber, price,
							startDate, startTime, endDate, endTime);
					ticketInfoDao.updateObjects(ticketInfo);
				} else {
					ticketInfo = new TicketInfo(ticketInfoName, event, type,
							ticketDescription, maxNumber, price, startDate,
							startTime, endDate, endTime);
					ticketInfoDao.insertObjects(ticketInfo);
				}
			}
	...
	public HttpServletRequest readMyEvents(HttpServletRequest request) {
		...
		List<Organizer> organizersList = new ReadGenericObjectService<>(
				Organizer.class).readListByField("user_ref=", userId);
		...
		for (Organizer organizer : organizersList) {
			List<Event> currentEvents = new ReadGenericObjectService<>(
					Event.class).readListByField("organizer_ref=",
					organizer.getId());
			for (Event event : currentEvents) {
				int allTicketsSold = 0;
				int allTickets = 0;
				try {
					if (!new ReadGenericObjectService<>(TicketInfo.class)
							.readListByField("event_ref=", event.getId())
							.isEmpty()) {
						allTicketsSold += new ReflectionDao<>(Ticket.class)
								.selectAggregateObjects("COUNT(event_ref)",
										"event_ref=", event.getId()).get(0)
								.intValue();
						allTickets += new ReflectionDao<>(TicketInfo.class)
								.selectAggregateObjects(
	...
	private Date combineDateTime(Date date, Date time) {
		...
		calendarA
				.set(Calendar.HOUR_OF_DAY, calendarB.get(Calendar.HOUR_OF_DAY));
		calendarA.set(Calendar.MINUTE, calendarB.get(Calendar.MINUTE));
		calendarA.set(Calendar.SECOND, calendarB.get(Calendar.SECOND));
		calendarA
				.set(Calendar.MILLISECOND, calendarB.get(Calendar.MILLISECOND));
	...
	public List<TicketInfo> readTicketInfo(int eventId) {
		List<TicketInfo> ticketsInfoList = new ReadGenericObjectService<>(
				TicketInfo.class).readListByField("event_ref=", eventId);
		;
		...
		while (iter.hasNext()) {
			...
			Date startDate = combineDateTime(currentTicketInfo.getStartDate(),
					currentTicketInfo.getStartTime());
			Date endDate = combineDateTime(currentTicketInfo.getEndDate(),
					currentTicketInfo.getEndTime());
	...
	public void deleteEvent(HttpServletRequest request, int id) {
		List<Ticket> ticketsList = new ReadGenericObjectService<>(Ticket.class)
				.readListByField("event_ref=", id);
		...
		ReflectionDao<TicketInfo> ticketInfoDao = new ReflectionDao<>(
				TicketInfo.class);
		List<TicketInfo> ticketInfoList = new ReadGenericObjectService<>(
				TicketInfo.class).readListByField("event_ref=", id);
		for (TicketInfo ticketInfo : ticketInfoList) {
			ticketInfoDao.deleteObjects(ticketInfo);
		}
		ReflectionDao<PromoCode> promoCodeDao = new ReflectionDao<>(
				PromoCode.class);
		List<PromoCode> promoCodeList = new ReadGenericObjectService<>(
				PromoCode.class).readListByField("event_ref=", id);
		for (PromoCode promoCode : promoCodeList) {
			promoCodeDao.deleteObjects(promoCode);
		}
		new ReflectionDao<>(Event.class)
				.deleteObjects(new ReadGenericObjectService<>(Event.class)
						.readById(id));
	}
	...
	public List<Event> search(HttpServletRequest request) {
		...
		if (!(request.getParameter("event_category") == null)
				&& !request.getParameter("event_category").equals("-1")) {
			query = query + ", category_ref=, "
					+ request.getParameter("event_category");
		}

		if (!(request.getParameter("event_type") == null)
				&& !request.getParameter("event_type").equals("-1")) {
			query = query + ", type_ref=, "
		...
		List<Event> events = new ReflectionDao<>(Event.class).selectObjects(2,
				queryToExecute);

		Iterator<Event> iter = events.iterator();

		if (!(request.getParameter("min_price") == null)
				&& !request.getParameter("min_price").equals("0")) {
			while (iter.hasNext()) {
				Event currentEvent = iter.next();
				List<TicketInfo> currentEventTicketsInfo = new ReflectionDao<>(
						TicketInfo.class).selectObjects(1, "event_ref=",
						currentEvent.getId(), "price>",
						request.getParameter("min_price"));
		...
		if (!(request.getParameter("max_price") == null)
				&& !request.getParameter("max_price").equals("0")) {
			while (iter.hasNext()) {
				Event currentEvent = iter.next();
				List<TicketInfo> currentEventTicketsInfo = new ReflectionDao<>(
						TicketInfo.class).selectObjects(1, "event_ref=",
